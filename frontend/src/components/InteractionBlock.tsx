    ...
    code({ node, inline, className, children, ...props }: any) {
      ...
      return !inline && match ? (
        <div className="markdown-code-block">
            <div className="code-block-header"> <span>{displayLang}</span> <button onClick={handleCopyMdCode} className="icon-button subtle small copy-button" title="Sao chép"><FiCopy /></button> </div>
...
const getLanguageForHighlighter = (ext?: string): string => {
    switch (ext?.toLowerCase()) {
        ...
        case 'fortios': return 'plaintext'; default: return 'plaintext';
    }
};
...
const InteractionBlock: React.FC<InteractionBlockProps> = React.memo(({
  ...
  const handleDownload = (filename: string, text: string | null | undefined) => {
    if (typeof text === 'string') {
      const element = document.createElement("a"); const file = new Blob([text], {type: 'text/plain;charset=utf-8'});
  ...
  const hasErrorSignal = (execData: any): boolean => {
      if (!execData) return false; const hasStdErr = !!execData?.error?.trim();
      const nonZeroReturn = execData?.return_code !== 0; const isFortiosError = execData?.executed_file_type === 'fortios' && execData?.return_code !== 0;
        ...
        if (isEditingThisBlock) {
            return (
                <textarea
                    ...
                    value={currentEditingCode ?? ''}
                    onChange={(e) => onUpdateEditingCode(e.target.value)}
                    disabled={isBusy}
                    spellCheck="false"
                    autoCapitalize="off"
                    autoComplete="off"
            ...
            if (codeStrFromData) {
                return ( <div className="code-block-container">
                       <div className="code-block-header">
                           ...
                           <div>
                               <button onClick={() => onToggleEditCode(id, codeStrFromData)} className="icon-button subtle small edit-code-button" title="Sửa mã" disabled={isBusy}><FiEdit /></button>
                               <button onClick={() => handleCopy(codeStrFromData)} className="icon-button subtle small" title="Sao chép" disabled={isBusy}><FiCopy /></button>
                               <button onClick={() => handleDownload(defaultFileName, codeStrFromData)} className="icon-button subtle small" title="Tải xuống" disabled={isBusy}><FiDownload /></button>
                           </div>
                       </div>
        ...
        return ( <div className="debug-content">
                ...
                {debugData?.explanation && ( <div className="markdown-content explanation-content"><h4>Giải thích & Đề xuất</h4><ReactMarkdown remarkPlugins={[remarkGfm]} components={MarkdownComponents}>{debugData.explanation}</ReactMarkdown></div> )}
                 ...
                 {correctedCode && ( <> <h4>Mã đề xuất</h4> <div className="code-block-container">
                            <div className="code-block-header"><span>{correctedCodeLang} (đã sửa)</span> <div><button onClick={() => handleCopy(correctedCode)} className="icon-button subtle small"><FiCopy /></button><button onClick={() => handleDownload(`corrected.${correctedCodeLang}`, correctedCode)} className="icon-button subtle small"><FiDownload /></button></div> </div>
  ...
  const renderIcon = () => { /* ... giữ nguyên ... */
   ...
   const renderActionButtons = (): JSX.Element[] => {
        ...
        const codeForAction = (type === 'ai-code' && isEditingThisBlock && currentEditingCode !== null)
                            ? currentEditingCode
                            : (type === 'ai-code' ? (data as string) : null);

        if (isEditingThisBlock) {
            actions.push(<button key="save" onClick={() => onSaveEditedCode(id)} disabled={isBusy || currentEditingCode === data} className="save-code"><FiSave /> Lưu Mã</button>);
            actions.push(<button key="cancel" onClick={onCancelEditCode} disabled={isBusy} className="cancel-edit"><FiXCircle /> Hủy</button>);
        } else if (type === 'ai-code' && codeForAction && codeForAction.trim()) {
            actions.push(<button key="review" onClick={() => onReview(codeForAction, id)} disabled={isBusy} title={`Đánh giá ${currentFileType ? `(.${currentFileType})` : ''}`}><FiEye /> Đánh giá</button>);
            ...
            actions.push(<button key="execute" onClick={() => onExecute(codeForAction, id)} disabled={isBusy} className="execute" title={execButtonText}>{execButtonIcon} {execButtonText}</button>);
        }
            ...
            if (executionBlockIndex > 0 && block.parentConversation) {
                ...
                if (precedingAICodeBlock) {
                    ...
                    if (editingBlockId === precedingAICodeBlock.id && currentEditingCode !== null) {
                        ...
                        codeThatFailedForDebug = precedingAICodeBlock.data as string;
                    }
                }
            }
            if (codeThatFailedForDebug) {
                 actions.push(<button key="debug" onClick={() => onDebug(codeThatFailedForDebug!, execResult, id)} disabled={isBusy} className="debug"><FiTool /> Gỡ lỗi</button>);
            }
        }
