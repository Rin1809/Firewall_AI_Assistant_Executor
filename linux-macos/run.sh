...
BACKEND_MODULE_PATH="backend/app.py" # Ktra su ton tai cua file app.py
...
if [ ! -f "$BACKEND_MODULE_PATH" ]; then
    echo "[LỖI] Không tìm thấy file backend: '$BACKEND_MODULE_PATH'."
    exit 1
fi
...
# Chay backend nhu mot module voi python -m
CMD_BACKEND="echo '>>> Dang kich hoat venv & chay Backend...'; source \"$VENV_ACTIVATE\" && python -m backend.app; echo '>>> Backend da dung hoac loi. Nhan Enter de dong cua so nay.'; read"
...
if [ -z "$TERM_CMD" ] && [ "$OS_TYPE" == "Linux" ]; then
    ...
    echo "       1. Chay: cd \"$PROJECT_ROOT\" && source \"$VENV_ACTIVATE\" && python -m backend.app"
